        ...
        <nav className="flex flex-row h-screen w-max relative reduceNav:ml-0 ml-[-4px]">
            <div className="pt-1 pb-6 h-full overflow-y-auto bg-backgroundBox flex flex-col justify-between select-none items-start">
                <div ref={topLinksRef} className="flex flex-col">
                    <Link draggable={false} onMouseEnter={() => setOpenedMenu(null)} href="/" className="flex flex-row gap-1 font-extrabold text-xl p-3 transition-colors hover:bg-backgroundBoxHover hover:text-foregroundHighlighted">
                ...
                <div className="flex flex-col text-sm font-bold w-full">
                    {botLinks.map((link, i) => {
                        return (
                            <Link draggable={false} key={i} target="_blank" href={link.href} className="flex flex-row gap-2 reduceNav:px-3 reduceNav:justify-start justify-center py-2 group hover:bg-backgroundBoxHover text-foregroundGrey hover:text-foregroundHighlighted transition-colors">
