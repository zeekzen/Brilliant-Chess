                    ...
                    return (
                        <div className="mb-[1px] flex flex-row items-end" key={i}>
                            {Array.from({ length: count }).map((_, i) => {
                                return <Image key={i} className={`${piece === PAWN ? 'mx-[2px]' : piece === ROOK || piece === KNIGHT || piece === BISHOP ? 'mx-[1px]' : ''} ${i !== 0 ? 'ml-[-10px]' : ''}`} priority alt={piece} width={piece === PAWN ? smallSize : piece === ROOK || piece === KNIGHT || piece === BISHOP ? mediumSize : bigSize} height={piece === PAWN ? smallSize : piece === ROOK || piece === KNIGHT || piece === BISHOP ? mediumSize : bigSize} style={{filter: `drop-shadow(1px 0 0 ${outlineColor}) drop-shadow(-1px 0 0 ${outlineColor}) drop-shadow(0 1px 0 ${outlineColor}) drop-shadow(0 -1px 0 ${outlineColor})`}} src={src} />
                            })}
                        </div>
                    )
                })}
            </div>
            <span className="text-foregroundGrey h-fit font-light text-[17px]">{white ? formatAdvantage(advantage) : formatAdvantage(-advantage)}</span>
        </div>
    )
}